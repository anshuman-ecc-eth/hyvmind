...
export function useGetUnvotedTokensForSwarm(swarmId: string) {
  ...
  return useQuery({
    ...
    queryFn: async () => {
      if (!actor || !identity) return { lawTokens: [], interpretationTokens: [] };
      ...
      const swarmLawTokens = graphData.lawTokens.filter(lt => locationIds.has(lt.parentLocationId));
      ...
      return { lawTokens: swarmLawTokens, interpretationTokens: swarmInterpTokens };
    },
...
export function useGetUserLawTokens() {
  ...
  return useQuery<LawToken[]>({
    queryKey: ['userLawTokens', identity?.getPrincipal().toText() ?? 'anonymous'],
    queryFn: async () => {
      if (!actor) return [];
      const owned = await actor.getMyOwnedGraphData();
...
export function useGetUserInterpretationTokens() {
  ...
  return useQuery<InterpretationToken[]>({
    queryKey: ['userInterpretationTokens', identity?.getPrincipal().toText() ?? 'anonymous'],
    queryFn: async () => {
      if (!actor) return [];
      const owned = await actor.getMyOwnedGraphData();
