  ...
  return (
    ...
    >
      <div className="mb-2 text-muted-foreground">
        Multiple matches found. Use ↑/↓ to select, Enter to confirm, Escape to cancel:
      </div>
      <div className="space-y-1">
        {candidates.map((candidate, index) => (
          ...
          >
            ...
            <span className="ml-2 text-muted-foreground">({candidate.type})</span>
            {candidate.parentContext && (
              <span className="ml-2 text-xs text-muted-foreground">in {candidate.parentContext}</span>
            )}
            <div className="text-xs text-muted-foreground truncate">{candidate.id}</div>
          </div>
        ))}
      </div>
    </div>
