...
export default function LandingGraphDiagram() {
  ...
  const { nodes, edges } = useMemo(() => {
    ...
    const getNodeRadius = (nodeType: string): number => {
      switch (nodeType) {
        case 'curation': return 12;
        case 'swarm': return 10;
        case 'location': return 8;
        case 'lawToken': return 6;
        case 'interpretationToken': return 6;
        default: return 6;
      }
    };
      ...
      graphData.swarms.forEach((swarm) => {
        if (nodeIdSet.has(swarm.parentCurationId) && nodeIdSet.has(swarm.id)) {
          ...
          const edgeExists = generatedEdges.some(
            e => (e.source === swarm.parentCurationId && e.target === swarm.id) ||
    ...
    if (graphData.locations) {
      graphData.locations.forEach((location) => {
        if (nodeIdSet.has(location.parentSwarmId) && nodeIdSet.has(location.id)) {
          // Check if edge already exists
          const edgeExists = generatedEdges.some(
            e => (e.source === location.parentSwarmId && e.target === location.id) ||
