...
type NodeType = 'all' | 'curation' | 'swarm' | 'location' | 'lawToken' | 'interpretationToken';
  ...
  return (
    ...
    >
      <div className="flex items-center gap-2 flex-wrap">
        ...
        {isSharedLawToken && (
          <Badge variant="default" className="text-xs flex items-center gap-1 bg-foreground text-background">
...
export default function FilterSortModal({
  ...
  const { attributeKeys, attributeValuesByKey, allTags } = useMemo(() => {
    ...
    if (graphData) {
      graphData.locations.forEach(loc => {
        loc.customAttributes.forEach(attr => {
          keys.add(attr.key);
      ...
      graphData.swarms.forEach(swarm => {
        swarm.tags.forEach(tag => tags.add(tag));
      });
    }
  ...
  const filteredAndSortedNodes = useMemo(() => {
    ...
    if (hasActiveFilters) {
      ...
      if (typeFilter === 'location' || typeFilter === 'lawToken' || typeFilter === 'interpretationToken') {
        filtered = filtered.filter(node => node.nodeType === typeFilter);
      }
      // When filtering by attributes, only show nodes with attributes (locations and interpretation tokens)
      if (attributeKey !== 'all' && attributeValue !== 'all') {
        filtered = filtered.filter(node => 
          node.nodeType === 'location' || node.nodeType === 'interpretationToken'
        );
      }
    ...
    return sorted;
  }, [allFlatNodes, typeFilter, attributeKey, attributeValue, selectedTags, sortOption, graphData]);
      ...
      <DialogContent className="max-w-5xl h-[80vh] flex flex-col">
        ...
        <div className="flex-shrink-0 grid grid-cols-3 gap-3 py-4">
          <div className="space-y-2">
            ...
            <Select value={typeFilter} onValueChange={(value) => setTypeFilter(value as NodeType)}>
          ...
          <div className="space-y-2">
            <label className="text-xs font-medium text-muted-foreground">Attribute Key</label>
            <Select 
              value={attributeKey} 
              onValueChange={(value) => {
          ...
          <div className="space-y-2">
            <label className="text-xs font-medium text-muted-foreground">Attribute Value</label>
            <Select 
        ...
        <div className="flex-shrink-0 pb-4 border-b">
          <div className="space-y-2">
            ...
            <div className="flex gap-2">
              ...
              <Button
                variant={sortOption === 'modificationDate' ? 'default' : 'outline'}
                size="sm"
              ...
              {filteredAndSortedNodes.length === 0 ? (
                ...
                <>
                  <div className="text-xs text-muted-foreground mb-2">
                    Showing {filteredAndSortedNodes.length} matching node{filteredAndSortedNodes.length !== 1 ? 's' : ''}
                  </div>
