...
export default function CreateLawTokenDialog({
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      toast.success(`Law Token added to "${side === 'yes' ? 'Yes' : 'No'}" side`);
      ...
      <DialogContent className="max-w-md max-h-[90vh] overflow-y-auto">
        ...
        <div className="space-y-4 py-1">
          ...
          <div className="space-y-1">
            <Label className="text-xs text-muted-foreground">Parent Location</Label>
            <Input
              value={sideLabel}
              disabled
        ...
        <DialogFooter>
          ...
          <Button onClick={handleSubmit} disabled={isSubmitting || !meaning.trim()}>
            {isSubmitting ? (
              <>
