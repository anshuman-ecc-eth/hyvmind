    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          ...
          <DialogDescription>
            Turn this Question of Law into an open-membership collectible badge. Anyone who mints it joins the swarm.
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="space-y-4 py-2">
          ...
          <div className="space-y-2">
            ...
            {imagePreview ? (
              ...
              >
                ...
                <span className="text-xs text-muted-foreground/60 mt-0.5">PNG, JPG, GIF up to 10MB</span>
              </button>
            )}
            <input
              ref={fileInputRef}
          ...
          {swarmTags.length > 0 && (
            <div className="space-y-1.5">
              ...
              <div className="flex flex-wrap gap-1.5">
                {swarmTags.map((tag) => (
                  <Badge key={tag} variant="outline" className="text-xs font-mono px-1.5 py-0">
                    {tag}
                  </Badge>
                ))}
              </div>
          ...
          <p className="text-xs text-muted-foreground border border-border rounded-md px-3 py-2 bg-muted/20">
            <span className="font-medium text-foreground">Unlimited copies</span> — this collectible functions as an open membership badge. There is no cap on the number of editions.
          </p>
        </div>
