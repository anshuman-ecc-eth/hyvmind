...
import { useGetAllLocations, useCreateLocation } from '../hooks/useQueries';
...
export default function CreateLocationDialog({ open, onOpenChange }: CreateLocationDialogProps) {
  ...
  const renderLocationTreeNode = (location: Location, level: number, tree: Map<string, Location[]>) => {
  ...
  const isDisabled = !title.trim() || createLocationMutation.isPending || !isReady;

  const locationTree = buildLocationTree(allLocations);
  const rootLocations = getRootLocations(allLocations);
      ...
      <DialogContent className="bg-white dark:bg-gray-950 max-w-2xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription className="text-sm">
            locations organize and contextualize your research content
          </DialogDescription>
        </DialogHeader>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="space-y-2">
            ...
            {metadata.length === 0 ? (
              <div className="text-xs text-muted-foreground bg-muted/30 p-3 rounded-md border border-dashed">
                No metadata. Add key-value pairs to describe this location.
              </div>
            ) : (
          ...
          <div className="space-y-2">
            ...
            {allLocations.length === 0 ? (
              <div className="text-xs text-muted-foreground bg-muted/30 p-3 rounded-md border border-dashed">
                No existing locations. This will be a root-level location.
              </div>
            ) : (
              <div className="max-h-64 overflow-y-auto border rounded-md p-3 space-y-2 bg-muted/30">
                {rootLocations.map(location => renderLocationTreeNode(location, 0, locationTree))}
              </div>
            )}
          </div>
