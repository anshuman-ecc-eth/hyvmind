import type { CustomAttribute, Tag, NodeId, backendInterface } from '../backend';
...
export function archiveNodeById(nodeId: string): void {
  ...
  archived.add(nodeId);
  localStorage.setItem(ARCHIVED_NODES_KEY, JSON.stringify(Array.from(archived)));
}
...
function formatMissingFieldsError(nodeType: string, missing: string[], example: string): string {
  return `Error: ${nodeType} creation requires: ${missing.join(', ')}.\nExample: ${example}`;
}

function formatSuccessMessage(nodeType: string, name: string, id: NodeId): string {
...
function formatInvalidTagTypeError(type: string, supportedTypes: string[]): string {
...
export async function executeCommand(
  ...
  switch (command) {
    ...
    case 'i':
      return await executeInterpretationTokenCommand(fields, createInterpretationToken);
    case 't':
      return {
        success: false,
        message: 'Error: Law tokens are auto-extracted from Location creation content and cannot be directly created via Terminal.',
      };
    default:
      return {
        success: false,
...
async function executeCurationCommand(
  ...
  if (missing.length > 0) {
    return {
      success: false,
      message: formatMissingFieldsError('Curation', missing, '/c name=X juris=IND'),
    };
  }

  // Validate jurisdiction as ISO 3166-1 alpha-3 code (case-insensitive)
  const jurisdictionUpper = jurisdiction.trim().toUpperCase();
  if (jurisdictionUpper.length !== 3 || !VALID_JURISDICTION_CODES.includes(jurisdictionUpper)) {
    return {
      success: false,
  ...
  if (fields.tags) {
    return {
      success: false,
      message: 'Error: Curations do not support tags. Remove the tags field and try again.',
    };
  }
...
async function executeSwarmCommand(
  ...
  if (missing.length > 0) {
    return {
      success: false,
      message: formatMissingFieldsError('Swarm', missing, '/s name=X parent=C123 tags=civil[string],2026[date]'),
    };
  }
  ...
  const tags: Tag[] = [];
  if (tagsValue) {
    const tagEntries = tagsValue.split(',').map((t) => t.trim()).filter((t) => t.length > 0);
...
async function executeLocationCommand(
  ...
  if ((!content || content.trim() === '') && (!tokensValue || tokensValue.trim() === '')) {
  ...
  if (missing.length > 0) {
    return {
      success: false,
      message: formatMissingFieldsError('Location', missing, '/l name=X parent=S123 content="..." or tokens={a}{b}{c}'),
    };
  }
  ...
  if (attributeValues) {
    const attrArray = Array.isArray(attributeValues) ? attributeValues : [attributeValues];
...
async function executeInterpretationTokenCommand(
  ...
  if (missing.length > 0) {
    return {
      success: false,
      message: formatMissingFieldsError('Interpretation Token', missing, '/i name=X from=L123 to=T456'),
    };
  }
  ...
  try {
    ...
    return {
      success: true,
      message: formatSuccessMessage('Interpretation Token', name.trim(), result),
    };
  } catch (error) {
    return {
      success: false,
...
function getFieldValue(fields: Record<string, string | string[]>, key: string): string {
