 ...
 */
export function generateOntologyMermaid(nodeId: string, graphData: GraphData): string {
  try {
    ...
    if (nodeInfo.type !== 'Curation') {
      ...
      if (parent) {
        ...
        if (!addedNodes.has(parentLocalName)) {
          lines.push(`    ${parentLocalName}["${parentShortLabel}"]${parentStyle}`);
    ...
    if (nodeInfo.type !== 'InterpretationToken') {
      ...
      for (const child of limitedChildren) {
        ...
        if (!addedNodes.has(childLocalName)) {
          lines.push(`    ${childLocalName}["${childShortLabel}"]${childStyle}`);
    ...
    if (nodeInfo.type === 'Location' || nodeInfo.type === 'LawToken' || nodeInfo.type === 'InterpretationToken') {
      ...
      for (const interpretation of limitedOutgoing) {
        ...
        const interpretationShortLabel = truncateLabel(interpretation.title, 18);
        
        if (!addedNodes.has(interpretationLocalName)) {
          lines.push(`    ${interpretationLocalName}{"${interpretationShortLabel}"}`);
          addedNodes.add(interpretationLocalName);
        }
        
        // From node -> Interpretation (solid line with FromRelation label)
        lines.push(`    ${localName} -->|FromRelation| ${interpretationLocalName}`);
        ...
        if (toNode) {
          ...
          lines.push(`    ${interpretationLocalName} -->|ToRelation| ${toLocalName}`);
        }
      }
    }
...
function getParentNode(nodeInfo: NodeInfo, graphData: GraphData): NodeInfo | null {
  switch (nodeInfo.type) {
    case 'Swarm':
      const swarm = nodeInfo.data as Swarm;
      const parentCuration = graphData.curations.find(c => c.id === swarm.parentCurationId);
      return parentCuration ? { id: parentCuration.id, name: parentCuration.name, type: 'Curation', data: parentCuration } : null;
      
    case 'Location':
      const location = nodeInfo.data as Location;
      const parentSwarm = graphData.swarms.find(s => s.id === location.parentSwarmId);
      return parentSwarm ? { id: parentSwarm.id, name: parentSwarm.name, type: 'Swarm', data: parentSwarm } : null;
      
    case 'LawToken':
      const lawToken = nodeInfo.data as LawToken;
      const parentLocation = graphData.locations.find(l => l.id === lawToken.parentLocationId);
      return parentLocation ? { id: parentLocation.id, name: parentLocation.title, type: 'Location', data: parentLocation } : null;
  ...
  switch (nodeInfo.type) {
    case 'Curation':
      ...
      for (const swarm of childSwarms) {
        children.push({ id: swarm.id, name: swarm.name, type: 'Swarm', data: swarm });
      }
      break;
    ...
    case 'Swarm':
      ...
      for (const location of childLocations) {
        children.push({ id: location.id, name: location.title, type: 'Location', data: location });
      }
      break;
    ...
    case 'Location':
      ...
      for (const lawToken of childLawTokens) {
        children.push({ id: lawToken.id, name: lawToken.tokenLabel, type: 'LawToken', data: lawToken });
      }
      break;
