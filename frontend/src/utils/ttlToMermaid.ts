...
export function ttlToMermaid(ttlText: string): string {
  ...
  for (const line of lines) {
    ...
    const fromRelationMatch = line.match(/hm:(\w+)\s+hm:FromRelation\s+hm:(\w+)/);
...
export function convertTTLToMermaid(ttlText: string, graphData?: GraphData): { mermaidText: string; mermaidError?: string } {
  try {
    ...
    return {
      mermaidText: '',
      mermaidError: error instanceof Error ? error.message : 'Unknown error generating Mermaid diagram',
    };
  }
}
