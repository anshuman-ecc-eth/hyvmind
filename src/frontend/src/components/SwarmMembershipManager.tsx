...
import { useGetGraphData, useGetSwarmsByCreator } from "../hooks/useQueries";
import SwarmMembershipApproval from "./SwarmMembershipApproval";

export default function SwarmMembershipManager() {
  const { data: createdSwarms, isLoading: isLoadingCreated } =
    useGetSwarmsByCreator();
    ...
    return (
      <Card>
        <CardHeader>
          ...
          <CardDescription>Co‑govern your research community.</CardDescription>
        </CardHeader>
        <CardContent className="pt-6 text-center">
          <p className="text-muted-foreground">
            You haven't joined or created any swarms yet. Join or create one to
            start collaborating.
          </p>
        </CardContent>
      </Card>
              ...
              <AccordionItem key={swarm.id} value={swarm.id}>
                <AccordionTrigger className="hover:no-underline">
                  <div className="flex items-center gap-3">
                    <Badge
                      variant="outline"
                      className="bg-chart-2/20 text-chart-2 border-chart-2/50"
                    >
                      Swarm
                    </Badge>
