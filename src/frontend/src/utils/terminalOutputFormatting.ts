...
export function formatTerminalOutput(
  text: string,
  _messageType: "success" | "error" | "example" | "command" | "ontology",
): LineToken[] {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (
      line.startsWith("  ") ||
      (prevToken &&
        (prevToken.type === "command" || prevToken.type === "heading"))
    ) {
 ...
 */
export function getMessageTypeEmoji(
  messageType: "success" | "error" | "example" | "command" | "ontology",
): string | undefined {
  switch (messageType) {
    case "success":
      return "✅";
