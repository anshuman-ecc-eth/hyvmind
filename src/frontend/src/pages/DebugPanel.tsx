                  ...
                  {graphData.locations.length === 0 ? (
                    ...
                    graphData.locations.map((location) => {
                      ...
                      const linkedLawTokens = graphData.edges
                        ...
                        .map((edge) =>
                          graphData.lawTokens.find((t) => t.id === edge.target),
                        )
                        .filter(Boolean);
                  ...
                  {graphData.lawTokens.length === 0 ? (
                    ...
                    graphData.lawTokens.map((lawToken) => {
                      const linkedLocations =
                        lawTokenToLocations.get(lawToken.id) || [];
                      const isShared = linkedLocations.length > 1;

                      return (
                        <Card
                          key={lawToken.id}
                          className={isShared ? "border-foreground" : ""}
                        >
                          <CardHeader>
                            <div className="flex items-center justify-between">
                              <CardTitle className="text-base">
                    ...
                    graphData.interpretationTokens.map(
                      (interpretationToken) => {
                        ...
                        const outgoingCount = graphData.edges.filter(
                          (e) => e.source === interpretationToken.id,
                        ).length;
                          ...
                          <Card key={interpretationToken.id}>
                            ...
                            <CardContent className="space-y-2">
                              ...
                              <div>
                                ...
                                <div className="space-y-1 mt-1">
                                  {interpretationToken.customAttributes.length >
                                  0 ? (
