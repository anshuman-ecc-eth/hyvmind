...
export default function TerminalPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (command === "config") {
      ...
      // /config ironclaw_token=<value>
      const icTokenMatch = rawInput.match(/\/config\s+ironclaw_token=(.+)/);
      if (icTokenMatch) {
        if (!isAdmin) {
          addMessage("error", "Not authorized. This command requires admin.");
          return;
        }
        const token = icTokenMatch[1].trim();
        if (!token) {
          addMessage("error", "Token cannot be empty.");
          return;
        }
        try {
      ...
      // /config ironclaw_url=<value>
      const icUrlMatch = rawInput.match(/\/config\s+ironclaw_url=(.+)/);
      if (icUrlMatch) {
        if (!isAdmin) {
          addMessage("error", "Not authorized. This command requires admin.");
          return;
        }
        const url = icUrlMatch[1].trim();
        if (!url) {
          addMessage("error", "URL cannot be empty.");
          return;
        }
        try {
          const resp = await fetch("/admin/url", {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({ url }),
          });
          if (resp.ok) {
            addMessage("success", `IronClaw URL set to: ${url}`);
          } else {
            const data = await resp.json().catch(() => ({}));
            addMessage("error", `Failed: ${data.error || resp.statusText}`);
          }
      ...
      // /config ironclaw_channel=<value>
      const icChMatch = rawInput.match(/\/config\s+ironclaw_channel=(.+)/);
      if (icChMatch) {
        if (!isAdmin) {
          addMessage("error", "Not authorized. This command requires admin.");
          return;
        }
        const channel = icChMatch[1].trim();
        if (!channel) {
          addMessage("error", "Channel cannot be empty.");
          return;
        }
        try {
          const resp = await fetch("/admin/channel", {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({ channel }),
          });
          if (resp.ok) {
            addMessage("success", `IronClaw channel set to: ${channel}`);
          } else {
            const data = await resp.json().catch(() => ({}));
            addMessage("error", `Failed: ${data.error || resp.statusText}`);
          }
      ...
      if (rawInput === "/config ironclaw_status") {
        if (!isAdmin) {
          addMessage("error", "Not authorized. This command requires admin.");
          return;
        }
        try {
          ...
          if (!resp.ok) {
            addMessage("error", `Proxy returned ${resp.status}`);
            return;
          }
