...
module {
    ...
    // Hyvmind patch: reduced from 16 to keep the runtime G-precompute table at
    // 64 points instead of 16384. WNAF is mathematically identical at a smaller
    // window (same result, more loop iterations); this avoids the multi-GiB
    // allocation churn that blew the canister's 2GiB Wasm heap at link time.
    public let WINDOW_G: Nat32 = 8;
    public let ECMULT_TABLE_SIZE_A: Nat = 8; //  1 << (WINDOW_A - 2);
    public let ECMULT_TABLE_SIZE_G: Nat = 64; // 1 << (WINDOW_G - 2);
    ...
    public class ECMultContext(
        ...
        public func ecmult(
            ...
            var z = Field.Field();
            let wnaf_na = Array.tabulateVar(256, func i = 0: Int32);
            let wnaf_ng = Array.tabulateVar(256, func i = 0: Int32);
    ...
    public func loadPrec(
        ...
        let res = Array.tabulate(64, func (j) {
